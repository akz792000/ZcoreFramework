...
(function($) {
	...
	$.mask = {
		...
		captureKey: '', //zandi
		//<ZANDI>
		formatBack : function(mask, value) {
			var 
				settings = {
					placeholder: "_",
					completed: null
			...
			buffer = buffer.toString().replace(new RegExp(',', 'g'), '').replace(new RegExp(settings.placeholder, 'g'), '');
			for (var i = 0; i < buffer.length; i++) {
				value = value.replace(new RegExp(buffer[i], 'g'), '');
			}
			return value;
		},
		format : function(mask, value) {
			var 
				settings = {
					placeholder: "_",
					completed: null
			...
			if (lastMatch + 1 < partialPosition) {
				...
				value = buffer.join('');
				return value.substring(0, lastMatch + 1);
			}
		}
		//</ZANDI>		
	};
...
function FloatFormat(formatStr) {
...
FloatFormat.prototype.formatBack = function (fNumStr) { // $1,223.06 --> 1223.06
	fNumStr += ''; //ensure it is string
	if (!fNumStr) return ''; //do not return undefined or null
	if (!isNaN(fNumStr)) return fNumStr;
	var fNumberStr = fNumStr;
	var negative = false;
	if (fNumStr.charAt(0) == '-') {
		fNumberStr = fNumberStr.substr(1);
...
FloatFormat.prototype.format = function (numStr, floatSize) { // 1223.06 --> $1,223.06
	// remove prefix, suffix and commas
	var numberStr = this.formatBack(numStr).toLowerCase();
	...
	for (var i = fracStr.length; i < floatSize; i++) { // if minFrac=4 then 1.12 --> 1.1200
		fracStr = fracStr + '0';
	}

	if (fracStr.length > floatSize) { // round
		//case 6143
		var num = new Number('0.' + fracStr);
		num = (floatSize == 0) ? Math.round(num) : num.toFixed(floatSize);
		// toFixed method has bugs on IE (0.7 --> 0)
		fracStr = num.toString(10).substr(2);
		var c = (num >= 1) ? 1 : 0; //carry
		var x, i = intStr.length - 1;
