...
jQuery.fn.contextPopup = function(menuData) {
	...
	var settings = {
		contextMenuClass: 'contextMenuPlugin ui-widget ui-widget-content ui-corner-all',
  ...
  function createMenu(e) {
    ...
    settings.items.forEach(function(item) {
      if (item) {
        var rowCode = '<li class="ui-state-default ui-corner-all"><a href="#"><span></span></a></li>';
        // if(item.icon)
        //   rowCode += '<img>';
        // rowCode +=  '<span></span></a></li>';
        var row = $(rowCode).appendTo(menu).hover(
        ...
        if (item.action) {
          row.click(function(){ item.action(e); });
        }
      } else {
