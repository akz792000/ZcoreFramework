...
$.extend(CalendarsPicker.prototype, {
	...
	commands: { // Command actions that may be added to a layout by name
		...
		today: {text: 'todayText', status: 'todayStatus', // Today's month
			...
			action: function(inst) {
				var curDate = inst.get('calendar').today();
				$.calendars.picker.showMonth(this, curDate.year(), curDate.month(), curDate.day()); 
			}
		},
	...
	defaultRenderer: {
		...
		picker: '<div class="calendars ui-widget ui-widget-content ui-helper-clearfix ui-corner-all">' +
		'<div class="calendars-nav ui-widget ui-widget-header ui-corner-tl ui-corner-tr">{link:prev}{link:today}{link:next}</div>{months}' +
		'{popup:start}<div class="calendars-ctrl ui-widget ui-widget-header ui-corner-bl ui-corner-br">{link:clear}{link:close}</div>{popup:end}' +
		...
		month: '<div class="calendars-month ui-widget ui-widget-content"><div class="calendars-month-header ui-widget ui-widget-content">{monthHeader}</div>' +
		'<table class="ui-widget ui-widget-content"><thead>{weekHeader}</thead><tbody>{weeks}</tbody></table></div>',
		// A week header: '{days}' to insert individual day names
		weekHeader: '<tr>{days}</tr>',
		// Individual day header: '{day}' to insert day name
		dayHeader: '<th class="ui-state-active">{day}</th>',
		// One week of the month: '{days}' to insert the week's days, '{weekOfYear}' to insert week of year
		week: '<tr>{days}</tr>',
		// An individual day: '{day}' to insert day value
		day: '<td class="ui-widget-content">{day}</td>',
		...
		selectedClass: 'calendars-selected ui-state-active',
		// Class for highlighted dates
		highlightedClass: 'calendars-highlight ui-state-highlight',
		// Class for today
		todayClass: 'calendars-today',
		// Class for days from other months
		otherMonthClass: 'calendars-other-month',
		// Class for days on weekends
		weekendClass: 'calendars-weekend ui-state-disabled',
		...
		disabledClass: 'calendars-disabled ui-state-disabled'
	},
	...
	_attachments: function(target, inst) {
		...
		var 
			trigger = inst.get('showTrigger'),
			triggerLastActive = null;
		//<ZANDI>
		/* 
		...
		*/
		inst.trigger = (!trigger ? $([]) :
			$(trigger).clone().addClass(this._triggerClass)
				['insertAfter'](target)
				.click(function() {
					if (!target.is(':disabled') && !target.is('[readonly]')) {
						$.calendars.picker[$.calendars.picker.curInst == inst ?	'hide' : 'show'](target[0]);
					}
				}))
				.bind("mousedown", function() {
					if (!target.is(':disabled') && !target.is('[readonly]')) {
						$(this).addClass('ui-state-active');
						triggerLastActive = this;
						var targetName = 'mouseup.' + target.parent().attr('id');
						$(document).unbind(targetName).bind(targetName, function(e){
							triggerLastActive = '';
							$(document).unbind(targetName);
						});
					}		
				})
	...
	_generateContent: function(target, inst) {
		...
		//<ZANDI_UI>
		picker.find('.calendars-cmd-prev').hover(
				function() { $(this).addClass(renderer.highlightedClass + ' ui-corner-tr'); },
				function() { $(this).removeClass(renderer.highlightedClass + ' ui-corner-tr'); } 
		);
		picker.find('.calendars-cmd-today').hover(
				function() { $(this).addClass(renderer.highlightedClass); },
				function() { $(this).removeClass(renderer.highlightedClass); } 
		);		
		picker.find('.calendars-cmd-next').hover(
				function() { $(this).addClass(renderer.highlightedClass + ' ui-corner-tl'); },
				function() { $(this).removeClass(renderer.highlightedClass + ' ui-corner-tl'); } 
		);
		picker.find('.calendars-cmd-clear').hover(
				function() { $(this).addClass(renderer.highlightedClass + ' ui-corner-br'); },
				function() { $(this).removeClass(renderer.highlightedClass + ' ui-corner-br'); } 
		);
		picker.find('.calendars-cmd-close').hover(
				function() { $(this).addClass(renderer.highlightedClass + ' ui-corner-bl'); },
				function() { $(this).removeClass(renderer.highlightedClass + ' ui-corner-bl'); } 
		);		
		//</ZANDI_UI>
	...
	_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader, calendar) {
		...
		var selector = '<select class="' + this._monthYearClass + ' calendars-month-year-m' +
		...
		if (yearRange == 'any') {
			...
			selector = '<select class="' + this._monthYearClass + ' calendars-month-year-y' +
				'" title="' + inst.get('yearStatus') + '">';
...
$.fn.calendarsPicker = function(options) {
	...
	if (options.container !== undefined) {
		options.container.bind('scroll.calendar', function() {	
			if ($.calendars.picker.curInst) {
				$.calendars.picker.hide($.calendars.picker.curInst, true);
			}
		});
...
(function($) {
	$.calendars.picker.regional['fa'] = {
		...
		clearText: 'حذف', clearStatus: 'پاک کردن تاريخ جاري',
