...
public class AbstractTaskletTest implements InitializeAware {

    private final Logger LOGGER = Logger.getLogger(AbstractTaskletTest.class);
    ...
    @Test
    public void normalLogErrorTaskletJobTest() throws Exception {
        final Job normalLogErrorTaskletJob = (Job) getApplicationContext().getBean("normalLogErrorTaskletJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        final JobExecution jobExecution = jobLauncher.run(normalLogErrorTaskletJob, jobParameters);
        stopWatch.stop();
        ...
        assertEquals("Tasklet.logError()", student.getName());
        assertEquals("Is Committed", student.getFamily());
    }

    @Test
    public void failedLogErrorTaskletJobTest() throws Exception {
        final Job failedLogErrorTaskletJob = (Job) getApplicationContext().getBean("failedLogErrorTaskletJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        final JobExecution jobExecution = jobLauncher.run(failedLogErrorTaskletJob, jobParameters);
        stopWatch.stop();
        LOGGER.info(String.format("Execute takes %d ms!", stopWatch.getLastTaskTimeMillis()));
