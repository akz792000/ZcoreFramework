...
@Scope("step")
@Component
public class SecondNormalReader extends DefaultItemReader<StudentEntity, StudentEntity> {

    private static final Log LOGGER = LogFactory.getLog(SecondNormalReader.class);
    ...
    protected List<StudentEntity> doRead(Integer loadLimit) throws Exception {
        if (!readFlag.getAndSet(true)) {
            LOGGER.info("SecondNormalReader.sortedRead() is called for the first time.");
            final List<StudentEntity> items = new ArrayList<>();
            items.add(new StudentEntity(1L, "Thread", "Is Read", 32));
            return items;
        }
        LOGGER.info("SecondNormalReader.sortedRead() is called for the second time.");
        return null;
    }

    @Override
    protected void logError(Integer readInterval, Throwable e) throws Exception {
        LOGGER.error("Unexpected method call of SecondNormalReader.logError()!");
