...
public class DefaultItemReaderTest implements InitializeAware {

    private final Logger LOGGER = Logger.getLogger(DefaultItemReaderTest.class);
    ...
    @Test
    public void chunkOrientedDefaultItemReaderJobTest() throws Exception {
        final Job chunkOrientedDefaultItemReaderJob = (Job) getApplicationContext().getBean("chunkOrientedDefaultItemReaderJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        final JobExecution jobExecution = jobLauncher.run(chunkOrientedDefaultItemReaderJob, jobParameters);
        stopWatch.stop();
