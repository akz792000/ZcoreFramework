...
public class DefaultNamedQueryItemReaderTest implements InitializeAware {

    private final Logger LOGGER = Logger.getLogger(DefaultNamedQueryItemReaderTest.class);
    ...
    @Test
    public void normalDefaultNamedQueryItemReaderJobTest() throws Exception {
        final Job normalDefaultNamedQueryItemReaderJob = (Job) getApplicationContext().getBean("normalDefaultNamedQueryItemReaderJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        final JobExecution jobExecution = jobLauncher.run(normalDefaultNamedQueryItemReaderJob, jobParameters);
        stopWatch.stop();
    ...
    @Test
    public void chunkOrientedDefaultNamedQueryItemReaderJobTest() throws Exception {
        final Job chunkOrientedDefaultNamedQueryItemReaderJob = (Job) getApplicationContext().getBean("chunkOrientedDefaultNamedQueryItemReaderJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        final JobExecution jobExecution = jobLauncher.run(chunkOrientedDefaultNamedQueryItemReaderJob, jobParameters);
        stopWatch.stop();
        LOGGER.info(String.format("Execute takes %d ms!", stopWatch.getLastTaskTimeMillis()));
