...
public class AbstractItemReaderTest implements InitializeAware {

    private final Logger LOGGER = Logger.getLogger(AbstractItemReaderTest.class);
    ...
    @Test
    public void normalLogErrorReaderJobTest() throws Exception {
        final Job normalLogErrorReaderJob = (Job) getApplicationContext().getBean("normalLogErrorReaderJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        final JobExecution jobExecution = jobLauncher.run(normalLogErrorReaderJob, jobParameters);
        stopWatch.stop();
        ...
        assertEquals("Reader.logError()", student.getName());
        assertEquals("Is Committed", student.getFamily());
    }

    @Test
    public void transactionalReaderJobTest() throws Exception {
        final Job transactionalPartitionerJob = (Job) getApplicationContext().getBean("transactionalReaderJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
    ...
    @Test
    public void failedLogErrorReaderJobTest() throws Exception {
        final Job failedLogErrorPartitionerJob = (Job) getApplicationContext().getBean("failedLogErrorReaderJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
