...
public class AbstractItemWriterTest implements InitializeAware {

    private final Logger LOGGER = Logger.getLogger(AbstractItemWriterTest.class);
    ...
    @Test
    public void normalLogErrorWriterJobTest() throws Exception {
        final Job normalLogErrorWriterJob = (Job) getApplicationContext().getBean("normalLogErrorWriterJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        final JobExecution jobExecution = jobLauncher.run(normalLogErrorWriterJob, jobParameters);
        stopWatch.stop();
        ...
        assertEquals("Writer.logError()", student.getName());
        assertEquals("Is Committed", student.getFamily());
    }

    @Test
    public void failedLogErrorWriterJobTest() throws Exception {
        final Job failedLogErrorWriterJob = (Job) getApplicationContext().getBean("failedLogErrorWriterJob");
        final JobParameters jobParameters = new JobParametersBuilder().addDate("today", new Date()).toJobParameters();
        StopWatch stopWatch = new StopWatch();
        stopWatch.start();
        final JobExecution jobExecution = jobLauncher.run(failedLogErrorWriterJob, jobParameters);
        stopWatch.stop();
        LOGGER.info(String.format("Execute takes %d ms!", stopWatch.getLastTaskTimeMillis()));
