...
public class ResourceServlet extends HttpServletBean {
	...
	private String zcoreJsPathPrefix = "/web-resources";  //<zandi/>
	...
	private Set allowedResourcePaths = new HashSet();
	{
		...
		allowedResourcePaths.add("/**/*.zrf"); //<zandi/>
		...
		allowedResourcePaths.add("META-INF/**/*.zrf"); //<zandi/>
	};
	...
	private Map defaultMimeTypes = new HashMap();
	{
		...
		defaultMimeTypes.put(".zrf", "application/xml"); //<zandi/>
	}
	...
	private Set compressedMimeTypes = new HashSet();
	{
		...
		// for compatible with T7 
		// appears that T7 changes text/javascript to application/javascript
		compressedMimeTypes.add("application/javascript");
		
		//<zandi>
		compressedMimeTypes.add("application/xml"); // to compress zrf file
		//</zandi>	
	}
	...
	private void prepareResponse(HttpServletResponse response, URL[] resources, String rawResourcePath)
		...
		for (int i = 0; i < resources.length; i++) {
			...
			log.info(resources[i].getPath() + " mimeType : " + currentMimeType); //<ZANDI>
	...
	private URL[] getRequestResourceURLs(HttpServletRequest request) throws MalformedURLException {
		...
		for (int i = 0; i < localResourcePaths.length; i++) {
			...
			//<zandi>
			if (resource == null) {
				resource = getServletContext().getResource(zcoreJsPathPrefix + localResourcePath);
			}
			//</zandi>			
			if (resource == null) {
